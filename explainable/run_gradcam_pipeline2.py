"""
Grad-CAM — Pipeline #2: DenseNet121 + ANOVA100->PCA5 + SGD (5-fold stratified CV).

Usage:
    conda run -n torch python term_preterm/explainable/run_gradcam_pipeline2.py
"""
import sys
...
def main():
    print("Grad-CAM — Pipeline #2: DenseNet121 + ANOVA100->PCA5 + SGD (5-fold CV)")
    ...
    extra_layers = list(MODEL_CONFIGS[MODEL_NAME]["extra"])

    out_dir = Path(__file__).resolve().parent / "output_gradcam" / "pipeline2"
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        model = DifferentiableModel(backbone, head, extra_layers)
        model.to(device)
        model.eval()
